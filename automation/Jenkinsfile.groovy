    ...
    stages {
        ...
        switch(env.MY_BRANCH) {                     // Main Switch is Required to be on top // Double Switch Cases is suggested // Using (IF else )
            ...
            case develop:
                ...
                env.NEW_LINE_VERSION = sh ( script: 'echo $((LINE_VERSION+1))', returnStdout: true).trim()      // Store New Suffix No. for StreamLine
            ...
            case release:
                ...
                env.TAG = sh ( script: 'echo "$STREAM-$CURRENT_MAX-rc-$NEW_LINE_VERSION"', returnStdout: true).trim()       //Generates New Tag for Release
                sh 'echo echo this is new tag version  ${TAG}'
            case preprod:
                ...
                env.TAG = sh ( script: 'echo "$STREAM-$CURRENT_MAX-pp-$NEW_LINE_VERSION"', returnStdout: true).trim()       //Generates New Tag for Pre-Prod
                sh 'echo echo this is new tag version  ${TAG}'
            case main:
                // Main Tag HINT = 1.0.0
